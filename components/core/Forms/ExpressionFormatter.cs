...
namespace AntDesign.Core.Forms;
...
internal static class ExpressionFormatter
{
    ...
    private static readonly ConcurrentDictionary<MemberInfo, CapturedValueFormatter> _capturedValueFormatterCache = new();
    private static readonly ConcurrentDictionary<MethodInfo, MethodInfoData> _methodInfoDataCache = new();

    public static void ClearCache()
    {
        _capturedValueFormatterCache.Clear();
        _methodInfoDataCache.Clear();
    }
    ...
    {
        if (!_methodInfoDataCache.TryGetValue(methodInfo, out var methodInfoData))
        {
            methodInfoData = GetMethodInfoData(methodInfo);
            _methodInfoDataCache[methodInfo] = methodInfoData;
        }
